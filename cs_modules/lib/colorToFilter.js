/* Baseado no código em:
   https://github.com/angel-rs/css-color-filter-generator/blob/master/index.js
*/

/**
 * Converts a color value to a CSS filter string that approximates the color.
 *
 * @param {string} input - The color input in HEX format (e.g., "#FF0000") or RGB format (e.g., "rgb(255, 0, 0)")
 * @returns {string} A CSS filter string that can be applied to elements to approximate the input color
 * @throws {Error} Throws "Invalid format!" if the input is not a valid HEX or RGB color format
 * @throws {Error} Throws "No suitable filter found!" if no filter with acceptable loss (<1) can be generated after 10 attempts
 *
 * @example
 * // Convert HEX color to filter
 * const filter = colorToFilter("#FF0000");
 *
 * @example
 * // Convert RGB color to filter
 * const filter = colorToFilter("rgb(255, 0, 0)");
 */
// eslint-disable-next-line no-unused-vars
function colorToFilter (input) {
  let rgb
